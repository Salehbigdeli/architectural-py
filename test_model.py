...
from model import OrderLine, Batch, OrderReference, OutOfStock, Quantity, Reference, Sku, allocate
...
def test_raise_out_of_stock_if_cannot_allocate():
    batch = Batch(Reference('batch-01'), Sku('Black-Chair'), 20)
    line = OrderLine(OrderReference('order-001'), Sku('Red-Chair'), 20)
    
    # cannot allocate 20 Red-Chairs to a batch of 20 Black-Chairs
    with pytest.raises(OutOfStock, match='Red-Chair'):
        allocate(line, [batch])
    
    allocate(OrderLine(OrderReference('order-002'), Sku('Black-Chair'), 20), [batch])
    
    # can't allocate more than available
    with pytest.raises(OutOfStock, match='Black-Chair'):
        allocate(OrderLine(OrderReference('order-002'), Sku('Black-Chair'), 1), [batch])